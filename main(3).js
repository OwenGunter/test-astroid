...
//draw the tiled background (make sure to do this first
        //so everything else is on top)
        for(var y = 0; y < 15; y++)
        {
                        for(var x = 0; x < 20; x++)
                        {
...
//draw all the asteroids
  ...
  //Stop Player leaving the screen
		...
		if(player.x > (SCREEN_WIDTH - player.width) + 45) // +45 because ship wasn't hitting edge of screen - stopped early

{
        ...
        if(player.x < 40)
		 ...
		 //player to go down
        if(player.y > (SCREEN_HEIGHT - player.height) + 45) //+45 b.cause ship wasn't hitting edge of screen - stopped early
 {
